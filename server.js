...
app.post("/api/generate-image",
  ...
  async (req, res) => {
    try {
      ...
      if (!appFile)  return res.status(400).json({ error: "Missing applicationImage (copy.png)" });
      if (!maskFile) return res.status(400).json({ error: "Missing maskImage (mask.png)" });
      ...
      // ---------- SECTION 1: "hold" the job application (left panel) ----------
      ...
      // ---------- SECTION 2: "wear" the mask (right panel) ----------
      ...
      let rightPanel = wearComposite;

      if (USE_AI) {
        try {
          const ai2 = await openai.images.edit({
            model: "gpt-image-1",
