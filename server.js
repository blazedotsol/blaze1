...
app.post("/api/generate-image",
  ...
  async (req, res) => {
    try {
      ...
      const appFile  = req.files?.templateImage?.[0];
      const maskFile = req.files?.templateImage?.[0];

      if (!userFile) return res.status(400).json({ error: "Missing userImage" });
      if ((mode === "dual" || mode === "hold") && !appFile) return res.status(400).json({ error: "Missing templateImage" });
      if ((mode === "dual" || mode === "wear") && !maskFile) return res.status(400).json({ error: "Missing templateImage" });
